...
@pytest.fixture
def db_connection():
    credentials = DBConfigHelper('fixtures/test_db_config.json').get_credentials()
...
def etl_obj(mocker, db_connection):
    ...
    def prepare_database(self):
        create_schema(self)
        db_connection.execute("create table if not exists hostel.Rooms "
                              "(id int auto_increment primary key not null , "
                              "name varchar(50) not null);")
        db_connection.execute("create table if not exists hostel.Students "
...
def test_can_create_schema_and_tables(etl_obj, db_connection):
    ...
    cursor = db_connection.execute("select schema_name from information_schema.SCHEMATA where SCHEMA_NAME='hostel';")
    result = cursor.fetchone()
    assert len(result) == 1
    assert result[0] == 'hostel'

    cursor = db_connection.execute("SELECT table_name FROM information_schema.tables WHERE table_name = 'Students';")
    result = cursor.fetchone()
    assert len(result) == 1

    cursor = db_connection.execute("SELECT table_name FROM information_schema.tables WHERE table_name = 'Rooms';")
    result = cursor.fetchone()
    assert len(result) == 1
...
def test_etl_export_results(etl_obj, db_connection):
    ...
    for query_name in queries['selects'].keys():
        assert os.path.exists(os.path.join(os.getcwd(), f"output/json/{query_name}.json"))
